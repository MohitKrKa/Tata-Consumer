...
const IndividualCard = () => {
  const {id}=useParams();
  let obj=board_leader[0];
  if(id<=8){
  obj=board_leader.find((leader)=>leader.id.toString()===id);
  }
  else{
    obj=leader_team.find((leader)=>leader.id.toString()===id);
  }
  return (
    <motion.div
      className="w-full max-w-6xl mx-auto my-10 bg-[#1461a5] rounded-2xl overflow-visible shadow-lg flex flex-col md:flex-row items-stretch relative justify-start py-10"
      initial="hidden"
      whileInView="visible"
